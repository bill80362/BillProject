...
class ChatLog
{
    ...
    static public function logReqData($Title,$Path,$Filename,$Msg = "",Array $DataArray=[] ){
        $log = new Logger($Title);
        $log->pushHandler(new StreamHandler($Path."/".$Filename, Logger::INFO));
        $log->info($Msg,$DataArray);
    }
}